...
Ниже перечислены известные алгоритмы консенсуса, которые **не реализованы** в текущей версии симулятора, но представляют значительный интерес для изучения.
...
Протокол atomic broadcast, лежащий в основе Apache ZooKeeper. Обеспечивает total order broadcast — все узлы видят сообщения в одинаковом порядке. Похож на Raft, но разработан независимо и раньше.

Ключевое отличие от Paxos: Zab гарантирует FIFO-порядок внутри сессии лидера и при смене лидеров (causal order), что критично для ZooKeeper как координационного сервиса.

**Источники:**
- Junqueira F., Reed B., Serafini M. "Zab: High-performance broadcast for primary-backup systems" (2011) — [IEEE DSN](https://doi.org/10.1109/DSN.2011.5958223)
- Reed B., Junqueira F. "A simple totally ordered broadcast protocol" (2008) — [ACM LADIS](https://www.datadoghq.com/pdf/zab.totally-ordered-broadcast-protocol.2008.pdf)
...
Leaderless-вариант Paxos с **оптимальной задержкой коммита** — один round-trip в большинстве случаев (когда нет конфликтов). В отличие от Multi-Paxos, не требует стабильного лидера, что устраняет single point of contention.

Команды без конфликтов коммитятся за fast path (1 RTT), конфликтующие — через slow path (2 RTT) с явным упорядочиванием через графы зависимостей.

**Источники:**
- Moraru I., Andersen D., Kaminsky M. "There is more consensus in Egalitarian Parliaments" (2013) — [ACM SOSP](https://www.cs.cmu.edu/~dga/papers/epaxos-sosp2013.pdf)
